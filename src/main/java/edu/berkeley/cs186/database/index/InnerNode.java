...
class InnerNode extends BPlusNode {
    ...
    public InnerNode emptyNode() {
        return new InnerNode(metadata, bufferManager, new ArrayList<>(), new ArrayList<>(), treeContext);
    }
    ...
    private InnerNode(BPlusTreeMetadata metadata, BufferManager bufferManager, Page page,
        ...
        try {
            ...
            assert (keys.size() == 0 || keys.size() + 1 == children.size());
    ...
    public BPlusNode getHelper(DataBox key) {
        List<DataBox> keys = this.keys;
        int whichone = InnerNode.numLessThanEqual(key, keys);
        return getChild(whichone);
    }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        // TODO I don't see why this is ever useful...
        LeafNode node = get(key);
        return node.put(key, rid);

    }

    public Optional<Pair<DataBox, Long>> my_put(DataBox splitkey, Long right_node_page_num) {
        int order = metadata.getOrder();
        if (keys.size() < metadata.getOrder() * 2) {
            int pos = InnerNode.numLessThanEqual(splitkey, keys);
            children.add(pos + 1, right_node_page_num);
            keys.add(splitkey);
            Collections.sort(keys);
            sync();
            return Optional.empty();
        } else { // do split
            InnerNode node0 = this;
            InnerNode node3 = emptyNode();
            ...
            DataBox splkey = keys.get(order);

            node3.keys = keys.stream().skip(order+1).toList();
            node3.children = children.stream().skip(order+1).toList();
            node0.keys = keys.stream().limit(order).toList();
            node0.children = children.stream().limit(order + 1).toList();

            node0.sync();
            node3.sync();

            return Optional.of(new Pair<DataBox,Long>(splkey, node3.page.getPageNum()));
        }
