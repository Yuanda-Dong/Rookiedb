...
public class BPlusTree {
    ...
    public Iterator<RecordId> scanAll() {
        ...
        LeafNode start = this.root.getLeftmostLeaf();
        return new Iterator<RecordId>() {
            private LeafNode curNode = start;
            private int curIdx = 0;

            public boolean hasNext() {
                if (curNode.getRightSibling().isEmpty() && curIdx == curNode.getKeys().size()) {
                    return false;
                }
                return true;
            }
            ...
            public RecordId next() {
                ...
                if (curIdx <= curNode.getKeys().size() - 1) {
                    RecordId result = curNode.getRids().get(curIdx);
                    curIdx += 1;
                    return result;
                } else {
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        int index = InnerNode.numLessThan(key, start.getKeys());

        return new Iterator<RecordId>() {
            private LeafNode curNode = start;
            private int curIdx = index;
    ...
    // def dfs(node,key) -> Option<Databox,Pageid>: // i.e. splitkey, and new node
    // if type(node) == leaf:
    // return node.put(key,rid)
    // next_node = node.get(key) // returns the next level node where key might
    // reside
    // if let Some(db,pg) = dfs(next_node,key):
    // return node.put(db,pg)
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        // root.get(key).put(key, rid);
        List<BPlusNode> path = new ArrayList<>();

        BPlusNode node = this.root;

        path.add(node);

        while (node instanceof InnerNode) {
            node = ((InnerNode) node).getHelper(key);
            path.add(node);
        }
        Collections.reverse(path);
        Optional<Pair<DataBox, Long>> z = Optional.empty();
        for (BPlusNode x : path) {
            if (x instanceof LeafNode) {
                ...
                z = ((InnerNode) x).my_put(z.get().getFirst(), z.get().getSecond());
                if (z.isEmpty()) {
                    return;
                }
            }
        ...
        if (!z.isEmpty()) { //
            ...
            List<Long> npages = new ArrayList<>();
            npages.add(this.root.getPage().getPageNum());
            npages.add(z.get().getSecond());
            nkeys.add(z.get().getFirst());
            InnerNode new_root = new InnerNode(metadata, bufferManager, nkeys, npages, lockContext);
